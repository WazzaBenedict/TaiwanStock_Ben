"""
V12.9.2 Multi-Portfolio Watchlist Manager
...
app = FastAPI(title="V12.9.2 Multi-Portfolio Watchlist Manager", version="12.9.2")
...
TW_DEFAULT_WEIGHTS={"technical":0.35,"fundamental":0.25,"chip":0.25,"news":0.15,
    "risk":0.10,"macro":0.05,"updated_at":"","version":"12.9.2","last_reason":"預設權重"}
...
async def _analyze_stock_mobile(stock_id: str) -> dict:
    """V12.9.2 mobile-first quote endpoint.
...
async def _analyze_stock_mobile_tech(stock_id: str) -> dict:
    """V12.9.2 mobile technical endpoint.
...
async def _fetch_us_mobile_tech(symbol: str) -> dict:
    """V12.9.2 US mobile technical endpoint: fast technical decision without full profile/chart work."""
    symbol = symbol.upper()
    try:
        async with httpx.AsyncClient() as cl:
...
TW_GROUP_MAP = {
    # V12.9.2: more granular Taiwan theme taxonomy so sector_spread is meaningful.
...
def select_tw_scan_pool(pool: str = "all", symbols: str = "", limit: int = 50) -> list[str]:
    """V12.9.2 Taiwan pool selector with true discovery mix.
...
def _repeat_penalty(sym: str, recent: set[str], current_category: str, previous_category: str = "") -> int:
    ...
    # V12.9.2: stronger repeat penalty so recent symbols stop dominating Watch/Fresh lists.
    return 20
...
async def _tw_scan_core(min_score:int=65,max_stocks:int=50,mode:str="full",pool_name:str="all",symbols:str="",use_cache:bool=True,diversity_mode:str="balanced",history_symbols:str=""):
    """V12.9.2 TW AI scan core — real diversity fix + safe mode parsing."""
    ...
    # V12.9.2: TW now uses the same diversity/history reranker as US.
...
def api_learning_summary_v128(payload: dict = Body(default={})):
    return {"version": "12.9.2", "summary": _learning_summary(payload), "time": datetime.now().isoformat()}

@app.post("/api/learning/calibrate")
def api_learning_calibrate_v128(payload: dict = Body(default={})):
    return {"version": "12.9.2", "calibration": _suggest_learning_weights(payload), "time": datetime.now().isoformat()}
...
def api_learning_apply_weights_v128(payload: dict = Body(default={})):
    ...
    if market == "tw":
        ...
        return {"version": "12.9.2", "applied": True, "market": market, "weights": saved, "time": datetime.now().isoformat()}
    return {"version": "12.9.2", "applied": False, "market": market, "weights": weights,
...
def api_learning_status_v128():
    ...
    return {"version": "12.9.2", "server_tw_signal_history": _lst(h),
...
def api_debug_learning_v128():
    return {"version": "12.9.2", "engine": "Real AI Learning Engine",
...
async def _us_scan_core(min_score:int=60,max_stocks:int=50,mode:str="full",pool_name:str="all",symbols:str="",use_cache:bool=True,diversity_mode:str="balanced",history_symbols:str=""):
    """V12.9.2 US AI scan core — real diversity fix + safe mode parsing."""
    t0=time.time()
    mode, mode_warn = _sanitize_scan_mode(mode)
    diversity_mode, div_warn = _sanitize_diversity_mode(diversity_mode)
...
def api_debug_recommendation_history():
    return {"version":"12.9.2","note":"Per-user recommendation history is stored client-side/localStorage or Firestore by the frontend. Backend diversified scan accepts history_symbols to apply novelty/repeat penalties.","fields":["symbol/stock_id","market","scan_category","shown_at","diversity_group","diversified_score"]}
...
def api_scan_status():
    ...
    def stat(cache):
        ...
        return out
    return {"version":"12.9.2","scan_cache_ttl":SCAN_CACHE_TTL,"tw_cache":stat(TW_SCAN_CACHE),"us_cache":stat(US_SCAN_CACHE)}
...
async def api_health_full():
    ...
    payload = {
        ...
        "version": "12.9.2",
        "frontend_expected": "12.9.2",
        "time": datetime.now().isoformat(),
        "backend": {
            "title": "V12.9.2 Multi-Portfolio Watchlist Manager",
            "version": "12.9.2"
        },
        "line": {
...
def api_performance_summary(payload: dict = Body(default={})):  # frontend sends Firestore journal rows
    ...
    return {"version": "12.9.2", "market": payload.get("market", "all"), "summary": _summarize_trades(trades), "time": datetime.now().isoformat()}

@app.post("/api/trade-journal/review")
def api_trade_journal_review(payload: dict = Body(default={})):
    trades = payload.get("trades") or []
    summary = _summarize_trades(trades)
    return {"version": "12.9.2", "review": summary.get("review"), "summary": summary, "time": datetime.now().isoformat()}
...
@app.get("/health")
def health():
    return{"status":"ok","version":"12.9.2","frontend_expected":"12.9.2","time":datetime.now().isoformat(),
           ...
           "features":["V12.9.2 Multi-Portfolio Watchlist Manager","AI Picker Pro","Mobile Fast Data",
